...
from cacp.gui.external.metric import SKLEARN_METRICS


class SklearnMetricsTable(html.Div):
    class ids:
        table = lambda aio_id: f"SklearnMetricsTable-table-{aio_id}"
        store = lambda aio_id: f"SklearnMetricsTable-store-{aio_id}"
        ...
        super().__init__([
            dag.AgGrid(
                ...
                columnDefs=[
                    ...
                    {"field": "docs_url", "cellRenderer": "markdown", "headerName": "Docs (based on)",
                     "maxWidth": None},
